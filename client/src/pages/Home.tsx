...
export default function Home() {
  ...
  const [guestbook, setGuestbook] = useState<{ name: string; status: string; message: string }[]>(() => { try { return JSON.parse(localStorage.getItem("nocturne-guestbook") || "[]"); } catch { return []; } });
  ...
  const submitRsvp = (e: React.FormEvent) => { e.preventDefault(); if (!rsvp.name.trim() || !rsvp.message.trim()) return; const next = [...guestbook, { ...rsvp, name: rsvp.name.trim(), message: rsvp.message.trim() }]; setGuestbook(next); localStorage.setItem("nocturne-guestbook", JSON.stringify(next)); setSent(true); setRsvp({ name: "", status: "Hadir", message: "" }); };
