...
function directionsUrl() {
  return `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(CONFIG.address)}`;
}
...
export default function Home() {
  ...
  return <div className={`site-shell ${opened ? "invitation-open" : ""}`}>
    ...
    <main id="top">
      ...
      <section id="event" className="event section-ink swipe-down reveal"><div className="section-index">03 <span>perayaan</span></div><div className="event-heading reveal swipe-up"><p className="eyebrow copper">Mark the evening</p><h2>Datang, duduk,<br /><em>dan rayakan kami.</em></h2></div><div className="event-grid"><div className="event-card reveal"><span className="event-number">01</span><h3>Akad nikah</h3><p className="event-time">16.00 <small>WIB</small></p><p>{CONFIG.dateLabel}<br />{CONFIG.venue}<br />{CONFIG.address}</p><div className="event-actions"><a href={directionsUrl()} target="_blank" rel="noreferrer" className="outline-link">Penunjuk arah <MapPin size={14} /></a><a href={CONFIG.mapsUrl} target="_blank" rel="noreferrer" className="outline-link">Lihat lokasi <ExternalLink size={14} /></a></div></div><div className="event-card reveal"><span className="event-number">02</span><h3>Resepsi</h3><p className="event-time">19.00 <small>WIB</small></p><p>{CONFIG.dateLabel}<br />{CONFIG.venue}<br />{CONFIG.address}</p><div className="event-actions"><a href={calendarUrl()} target="_blank" rel="noreferrer" className="outline-link">Add to Calendar <CalendarDays size={14} /></a><a href={directionsUrl()} target="_blank" rel="noreferrer" className="outline-link">Penunjuk arah <MapPin size={14} /></a></div></div></div><div className="countdown reveal"><p className="eyebrow copper">Menuju hari yang kami tunggu</p><div className="countdown-row">{[[countdown.days, "hari"], [countdown.hours, "jam"], [countdown.minutes, "menit"], [countdown.seconds, "detik"]].map(([num, label]) => <div key={label}><strong>{String(num).padStart(2, "0")}</strong><span>{label}</span></div>)}</div></div></section>
